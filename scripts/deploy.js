...
async function main() {
    const [deployer, admin, keeper, strategyManager, user1] = await ethers.getSigners();
    ...
    const VaultStrategyRouter = await ethers.getContractFactory("VaultStrategyRouter", deployer);
    const router = await VaultStrategyRouter.deploy(assetTokenAddress, admin.address, keeper.address);
    ...
    const BabylonStrategy = await ethers.getContractFactory("BabylonStrategy", deployer);
    const babylonStrategy = await BabylonStrategy.deploy(assetTokenAddress, strategyManager.address);
    ...
    const BounceBitStrategy = await ethers.getContractFactory("BounceBitStrategy", deployer);
    const bounceBitStrategy = await BounceBitStrategy.deploy(assetTokenAddress, strategyManager.address);
    ...
    await assetToken.connect(admin).transfer(user1.address, ethers.parseEther("1000"));
