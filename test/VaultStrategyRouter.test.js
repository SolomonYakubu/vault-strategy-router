...
describe("VaultStrategyRouter - Professional", function () {
  async function deployProfessionalFixture() {
    const [deployer, admin, keeper, strategyManager, user1, user2] =
      await ethers.getSigners();

    const MockStBTC = await ethers.getContractFactory("MockStBTC");
    const asset = await MockStBTC.deploy("stBTC", "stBTC", admin.address);
    ...
    await babylonStrategy
      .connect(strategyManager)
      .setRouter(await router.getAddress());
    await bounceBitStrategy
      .connect(strategyManager)
      .setRouter(await router.getAddress());

    await router
      .connect(strategyManager)
      .addStrategy(await babylonStrategy.getAddress());
    await router
      .connect(strategyManager)
      .addStrategy(await bounceBitStrategy.getAddress());
  ...
  describe("Access Control", function () {
    it("Should only allow ADMIN_ROLE to pause/unpause", async function () {
      ...
      await expect(router.connect(user1).pause()).to.be.revertedWithCustomError(
        router,
    ...
    it("Should only allow STRATEGY_MANAGER_ROLE to add strategies", async function () {
      const { router, user1, strategyManager, asset } =
        await deployProfessionalFixture();
      const BabylonStrategy = await ethers.getContractFactory(
        "BabylonStrategy"
      );
      const tempStrategy = await BabylonStrategy.deploy(
        await asset.getAddress(),
        strategyManager.address
      );
      await tempStrategy
      ...
      await expect(
        router.connect(user1).addStrategy(await tempStrategy.getAddress())
      ).to.be.revertedWithCustomError(
        router,
        "AccessControlUnauthorizedAccount"
      );
      await expect(
        router
          .connect(strategyManager)
          .addStrategy(await tempStrategy.getAddress())
  ...
  describe("Deposit/Withdraw", function () {
    it("Should revert deposit of 0 amount", async function () {
      ...
      await expect(
        router.connect(user1).deposit(0)
    ...
    it("Should allow deposit and emit event", async function () {
      ...
      await asset
        .connect(user1)
        .approve(router.getAddress(), ethers.parseEther("100"));
      await expect(
        router.connect(user1).deposit(ethers.parseEther("100"))
    ...
    it("Should revert withdraw of 0 shares", async function () {
      const { router, user1 } = await deployProfessionalFixture();
      await expect(
        router.connect(user1).withdraw(0)
      ).to.be.revertedWithCustomError(router, "AmountMustBePositive");
    });
    ...
    it("Should revert withdraw of more shares than owned", async function () {
      const { router, user1 } = await deployProfessionalFixture();
      await expect(
        router.connect(user1).withdraw(1000)
    ...
    it("Should allow deposit and withdraw, emitting events", async function () {
      ...
      await asset
        .connect(user1)
        .approve(router.getAddress(), ethers.parseEther("10"));
      await router.connect(user1).deposit(ethers.parseEther("10"));
      await expect(
        router.connect(user1).withdraw(ethers.parseEther("10"))
  ...
  describe("Rebalancing", function () {
    it("Should not rebalance if no strategy outperforms by threshold", async function () {
      ...
      await expect(router.connect(keeper).runKeeperTasks()).to.not.emit(
        router,
        "Rebalanced"
      );
    });
    it("Should rebalance if a strategy outperforms by threshold", async function () {
      ...
      await bounceBitStrategy
        .connect(strategyManager)
        .setPerformanceMetric(600);
      // Advance time by 8 days
      await ethers.provider.send("evm_increaseTime", [8 * 24 * 60 * 60]);
      await ethers.provider.send("evm_mine");
      // Now run keeper to update APY cache and trigger rebalance
      await expect(router.connect(keeper).runKeeperTasks()).to.emit(
        router,
        "Rebalanced"
      );
    });
  ...
  describe("Emergency & Pause", function () {
    it("Should allow ADMIN_ROLE to emergency withdraw and pause", async function () {
      ...
      await expect(
        router.connect(admin).emergencyWithdrawFromAllStrategies()
    ...
    it("Should not allow deposit when paused", async function () {
      ...
      await asset
        .connect(user1)
        .approve(router.getAddress(), ethers.parseEther("1"));
      await expect(router.connect(user1).deposit(ethers.parseEther("1"))).to.be
        .reverted; // Accept any revert, as custom error is used
    });
  });
  ...
  describe("Advanced & Edge Cases", function () {
    it("Should emit correct event args on deposit and withdraw", async function () {
      ...
      await asset
        .connect(user1)
        .approve(router.getAddress(), ethers.parseEther("5"));
    ...
    it("Should handle multi-user deposits and withdrawals correctly", async function () {
      ...
      await asset
        .connect(user2)
        .approve(router.getAddress(), ethers.parseEther("20"));
      await router.connect(user1).deposit(ethers.parseEther("10"));
      ...
      await expect(
        router.connect(user2).withdraw(ethers.parseEther("20"))
      ).to.emit(router, "Withdrawn");
    });
    it("Should allow withdraw from a deactivated strategy", async function () {
      const { router, asset, user1, strategyManager, babylonStrategy } =
        await deployProfessionalFixture();
      await asset
        .connect(user1)
        .approve(router.getAddress(), ethers.parseEther("5"));
      ...
      await router
        .connect(strategyManager)
        .removeStrategy(await babylonStrategy.getAddress());
      await expect(
        router.connect(user1).withdraw(ethers.parseEther("5"))
      ).to.emit(router, "Withdrawn");
    });
    it("Should not allow revoked role to add strategies", async function () {
      const { router, admin, strategyManager, asset } =
        await deployProfessionalFixture();
      const BabylonStrategy = await ethers.getContractFactory(
        "BabylonStrategy"
      );
      ...
      await router
        .connect(admin)
        .revokeRole(
          await router.STRATEGY_MANAGER_ROLE(),
          strategyManager.address
        );
      await expect(
        router
    ...
    it("Should revert if adding the same strategy twice", async function () {
      const { router, strategyManager, babylonStrategy } =
        await deployProfessionalFixture();
      await expect(
        router
          .connect(strategyManager)
    ...
    it("Should emergency withdraw from both strategies", async function () {
      const { router, asset, user1, user2, admin } =
        await deployProfessionalFixture();
      await asset
        .connect(user1)
        .approve(router.getAddress(), ethers.parseEther("10"));
